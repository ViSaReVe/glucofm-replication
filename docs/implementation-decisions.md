...
8. **Augmentation.** Probabilities/ranges and probability decay follow C.7. The baseline phase is uniform over a cycle; decimation retains a random grid offset modulo three. A degenerate corruption that removes every observation is reverted. All randomness is regenerated per training access and seeded for a run.
